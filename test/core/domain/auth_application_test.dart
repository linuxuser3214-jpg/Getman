...
void main() {
  ...
  AuthApplication apply(AuthConfig auth, {Map<String, String>? headers, String Function(String)? resolve}) =>
  ...
  group('bearer', () {
    ...
    test('runs the token through resolve', () {
      final app = apply(const AuthConfig(type: AuthType.bearer, token: '{{tok}}'),
          resolve: (v) => v == '{{tok}}' ? 'real' : v);
      expect(app.headers, {'Authorization': 'Bearer real'});
    });

    test('skips when the token resolves empty', () {
      expect(apply(const AuthConfig(type: AuthType.bearer, token: '')).headers, isEmpty);
    });
  ...
  group('basic', () {
    test('base64-encodes user:pass', () {
      final app = apply(const AuthConfig(type: AuthType.basic, username: 'u', password: 'p'));
      final value = app.headers['Authorization']!;
      expect(value.startsWith('Basic '), isTrue);
      expect(utf8.decode(base64.decode(value.substring('Basic '.length))), 'u:p');
    });
  ...
  group('apiKey', () {
    test('header location adds the named header', () {
      final app = apply(const AuthConfig(
          type: AuthType.apiKey, apiKeyName: 'X-Key', apiKeyValue: 'secret'));
    ...
    test('header location skips when that header already exists', () {
      final app = apply(
          const AuthConfig(type: AuthType.apiKey, apiKeyName: 'X-Key', apiKeyValue: 'secret'),
    ...
    test('skips entirely when the name resolves empty', () {
      final app = apply(const AuthConfig(type: AuthType.apiKey, apiKeyName: '', apiKeyValue: 'v'));
      expect(app.headers, isEmpty);
      expect(app.queryParam, isNull);
    });
    ...
    test('resolves both name and value', () {
      final app = apply(
        const AuthConfig(type: AuthType.apiKey, apiKeyName: '{{n}}', apiKeyValue: '{{v}}'),
