...
void main() {
  ...
  int leafCount(InlineSpan span) {
    ...
    span.visitChildren((child) {
      if (child is TextSpan && (child.children == null || child.children!.isEmpty)) n++;
      return true;
    });
    return n;
  }
  ...
  group('VariableHighlightController', () {
    test('highlights {{var}} tokens once colors are pushed', () {
      ...
      )..updateColors(resolved: const Color(0xFF00FF00), unresolved: const Color(0xFFFF0000));
    ...
    test('reuses the cached match list across repaints with unchanged text', () {
    ...
    test('recomputes when the text changes', () {
      ...
      c.text = 'now {{a}} and {{b}}';
      // "now " + {{a}} + " and " + {{b}} = 4 leaf spans (ends on a token, no tail)
      expect(leafCount(buildSpan(c)), 4);
    });

    test('toggling a variable to resolved keeps token boundaries, flips color', () {
      ...
      Color? tokenColor(InlineSpan span) {
        Color? found;
        span.visitChildren((child) {
          if (child is TextSpan && child.text == '{{token}}') found = child.style?.color;
          return true;
        });
        return found;
      }
