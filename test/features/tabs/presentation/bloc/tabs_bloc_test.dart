...
void main() {
  ...
  group('reaction emission', () {
    test('sendStarted fires first when a send begins', () async {
      await loadWith([tab('a')]);
      final baseSeq = bloc.state.reactionSeq;

      // Completer lets us resolve the send after capturing the first emission.
      final completer = Completer<HttpResponseEntity>();
      stubSend(() => completer.future);

      final emitted = <ThemeReaction>[];
      final sub = bloc.stream.listen((s) {
        if (s.reactionSeq > baseSeq + emitted.length) {
          if (s.lastReaction != null) emitted.add(s.lastReaction!);
        }
      });

      bloc.add(const SendRequest(tabId: 'a'));
      // Wait for the sendStarted reaction to fire (before the future resolves).
      await expectLater(
        bloc.stream,
        emitsThrough(
          predicate<TabsState>(
            (s) =>
                s.lastReaction?.kind == ThemeReactionKind.sendStarted &&
                s.reactionSeq > baseSeq,
          ),
        ),
      );

      expect(
        bloc.state.lastReaction?.kind,
        ThemeReactionKind.sendStarted,
      );
      expect(bloc.state.reactionSeq, greaterThan(baseSeq));

      // Clean up: resolve the future and let the bloc settle.
      completer.complete(
    ...
    test(
      'success: 200 response → reactionKind success + correct metadata',
      () async {
        await loadWith([tab('a')]);
        final baseSeq = bloc.state.reactionSeq;
        ...
        expect(reaction.kind, ThemeReactionKind.success);
        expect(reaction.statusCode, 200);
        expect(reaction.durationMs, 42);
        // sendStarted + success = 2 bumps from baseline
        expect(bloc.state.reactionSeq, baseSeq + 2);
      },
    );

    test(
      'clientError: 404 NetworkFailure → reactionKind clientError',
      () async {
        await loadWith([tab('a')]);
        final baseSeq = bloc.state.reactionSeq;
        ...
        final reaction = bloc.state.lastReaction!;
        expect(reaction.kind, ThemeReactionKind.clientError);
        expect(reaction.statusCode, 404);
        expect(bloc.state.reactionSeq, baseSeq + 2);
      },
    );

    test(
      'serverError: 503 NetworkFailure → reactionKind serverError',
      () async {
        await loadWith([tab('a')]);
        final baseSeq = bloc.state.reactionSeq;
        ...
        final reaction = bloc.state.lastReaction!;
        expect(reaction.kind, ThemeReactionKind.serverError);
        expect(reaction.statusCode, 503);
        expect(bloc.state.reactionSeq, baseSeq + 2);
      },
    );

    test(
      'networkError: null-statusCode non-cancelled NetworkFailure → '
      'reactionKind networkError',
      () async {
        await loadWith([tab('a')]);
        final baseSeq = bloc.state.reactionSeq;
    ...
    test(
      'cancelled: NetworkFailureType.cancelled → reactionKind cancelled',
      () async {
        await loadWith([tab('a')]);
        final baseSeq = bloc.state.reactionSeq;
        ...
        final reaction = bloc.state.lastReaction!;
        expect(reaction.kind, ThemeReactionKind.cancelled);
        expect(bloc.state.reactionSeq, baseSeq + 2);
      },
    );

    test(
      'unexpected error (non-NetworkFailure) → reactionKind networkError',
      () async {
        await loadWith([tab('a')]);
        final baseSeq = bloc.state.reactionSeq;
        ...
        final reaction = bloc.state.lastReaction!;
        expect(reaction.kind, ThemeReactionKind.networkError);
        // sendStarted + networkError = 2 bumps from baseline
        expect(bloc.state.reactionSeq, baseSeq + 2);
      },
    );
  });
