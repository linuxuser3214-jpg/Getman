...
void main() {
  ...
  group('addToHistory – dedup', () {
    test('same method+url+body is deduplicated: only one entry remains, moved to newest position',
        () async {
      final first = makeConfig(url: 'https://a.com');
      final second = makeConfig(url: 'https://b.com');
      final duplicate = makeConfig(url: 'https://a.com'); // same signature as first

      await dataSource.addToHistory(first, 10);
      await dataSource.addToHistory(second, 10);
      await dataSource.addToHistory(duplicate, 10); // should evict first, re-insert at end
    ...
    test(
        'different headers on same method+url+body ARE deduplicated — headers are not part of the dedup key',
        () async {
      // Headers are intentionally excluded from the dedup signature per CLAUDE.md §4.4.
      // Two requests that differ only in headers share the same method+url+body signature
      // and are therefore treated as the same logical request.
      ...
      // Both share the same signature; the second replaces the first → 1 entry.
      final history = await dataSource.getHistory();
      expect(history, hasLength(1));
    });

    test('two different configs (different URLs) both remain in the box', () async {
    ...
    test('two different configs (different methods) both remain in the box', () async {
      final getReq = makeConfig(method: 'GET', url: 'https://example.com');
      final postReq = makeConfig(method: 'POST', url: 'https://example.com');
  ...
  group('addToHistory – trim', () {
    ...
    test('actually shrinks below a lowered limit when called again', () async {
      // Fill to 5 entries.
      for (var i = 0; i < 5; i++) {
        await dataSource.addToHistory(makeConfig(url: 'https://item-$i.com'), 10);
      }
      expect(box.length, 5);
