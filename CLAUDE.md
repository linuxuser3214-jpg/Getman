...
- **Adding or changing a theme: read [`docs/THEME_AUTHORING.md`](docs/THEME_AUTHORING.md) first.** A theme is not just colors/shapes — since the reactive-motion work it also has a personality in motion (a 7th extension, `AppMotion`: `reactionOverlay` + `sendAffordance`, driven by the `ThemeReaction` spine in `lib/core/theme/motion/`), plus optional animated `scaffoldBackground` ambient, a theme-switch transition, and opt-in sound. The guide is the per-theme reactive checklist; future visual/motion ideas to fold into themes live in [`docs/BACKLOG.md`](docs/BACKLOG.md) under **🎨 Themes, Visuals & Motion**. Mechanically: drop `<name>_theme.dart` (+ `<name>_palette.dart`/`<name>_decorations.dart`/`<name>_motion.dart`, and optionally `<name>_components.dart` for custom widget slots) under `lib/core/theme/themes/<name>/`, export `ThemeData <name>Theme(Brightness, {bool isCompact, bool reduceEffects})` attaching all extensions (incl. `<name>Motion(reduceEffects:)` and either `defaultAppComponents()` or your own `<name>Components()`), and register a `ThemeDescriptor` in `theme_registry.dart`'s `appThemes` map with a new ID constant in `theme_ids.dart`. A theme may compose an external Material kit as its base `ThemeData` (AURIS composes `AurisTheme.dark()/.light()` and spreads `...base.extensions.values` so the kit's own extension survives `copyWith`). No widget edits are required — widgets consume the extensions uniformly. Everything heavy must degrade to identity/static under `reduceEffects`. **Reuse the shared `lib/core/theme/motion/` helpers** rather than re-deriving semantics: `flavorFor`/`StatusReactionFlavor` (HTTP-status **and** transport-failure classification — transport failures ride on `ThemeReaction.transportFailure`, a theme-local `TransportFailureKind` mapped from `NetworkFailureType` inside `TabsBloc` so the spine never imports `core/error`), `latencyWeight`/`inFlightTension`, and `photosensitivity.dart` (`safeFlashCount` / `kMaxSafeFlashesPerSecond` — a WCAG 2.3.1 3 Hz cap any *repeating* flash must route through; see THEME_AUTHORING §5b).
