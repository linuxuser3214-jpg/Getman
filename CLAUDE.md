...
6. `Shortcuts` + `Actions` wire global keyboard intents: **Ctrl/Cmd+N** new tab, **+W** close tab, **+S** save, **+Enter** send, **+B** beautify JSON, **+K** command palette, **+L** focus URL, **Ctrl+Tab / Ctrl+Shift+Tab** next/prev tab, **Cmd/Ctrl+1–9** jump to tab. The shortcut map is the computed `appShortcuts` in `main.dart` (a built map, not a `const` literal — the digit→`JumpToTabIntent` bindings are generated in a loop; `@visibleForTesting`). Tab-switch + focus-URL `Action`s live in `MainScreen` (they need `activeIndex`/`tabs`/`UrlFocusRegistry`).
...
- The response pane is a shell (`response_section.dart` → `ResponseSection`) over per-tab views under `tabs/presentation/widgets/response/` (`ResponseBodyView` + private `_PrettyRawToggle`, `ResponseHeadersView`, `ResponseCookiesView`, `ResponseTestsView`, `ResponseMetadataItem`). `ResponseBodyView` carries Copy + Save-to-file (`saveJsonFileWithFeedback`, json/txt) over `_copyableText()` (the verbatim body incl. the large-body cache).
...
- **Decorations**:
  ...
  - `context.appDecoration.wrapInteractive(child: …, onTap: …, scaleDown: …)` — tap animation wrapper (brutalist = scale-bounce via internal `BrutalBounce`). Never reference `BrutalBounce` directly outside `lib/core/theme/themes/brutalist/`.
...
- **Keyboard shortcuts**: the global activator→intent map is `appShortcuts` in `main.dart` (computed, `@visibleForTesting`); `Action`s are split by where their dependencies live. `NewTabIntent`/`CommandPaletteIntent` Actions are at the root; `CloseTabIntent`, `SendRequestIntent`, `NextTabIntent`/`PrevTabIntent`/`JumpToTabIntent`, and `FocusUrlIntent` at `MainScreen` (they need `activeIndex`/`tabs`/env-resolution/`UrlFocusRegistry`); `SaveRequestIntent` and `BeautifyJsonIntent` inside `RequestView`. Put intents where `context.read<TabsBloc>()` is reachable.
